...
export const getVotes = async () => {
  const resp = await axiosInstance.get(baseURL + "Votes/AllVotes");
  const data = await resp.data;
  return data;
};
...
export const deleteVote = async (recipeId, userId) => {
  const resp = await axiosInstance.delete(
    baseURL + `Votes/Vote/${recipeId}-${userId}`
  );
  return await resp.status;
};

export const postVote = async (post) => {
  const resp = await axiosInstance.post(baseURL + "Votes/Vote", post);
  return await resp.status;
};
